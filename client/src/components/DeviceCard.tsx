...
export interface DeviceCardProps {
  ...
  name: string;
  type: string;
  status: 'active' | 'warning' | 'error' | 'offline'; // Add 'offline' status
  lastUpdate?: string;
  batteryLevel?: number;
  imageUrl?: string;
  associatedPetName?: string; // Add associatedPetName prop
}

function DeviceStatus({ status }: { status: 'active' | 'warning' | 'error' | 'offline' }) {
  const statusConfig = {
    ...
    offline: { label: 'Offline', className: 'bg-gray-500 text-white hover:bg-gray-600' }, // Add offline status style
  };
...
export default function DeviceCard({
  ...
  return (
    <Link href={`/devices/${id}`}> {/* Wrap entire card with Link */}
      <Card className="card-device hover:shadow-lg transition-all border-none cursor-pointer"> {/* Add cursor-pointer */}
        <CardContent className="p-6">
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {associatedPetName && <p className="text-xs opacity-70">Mascota: {associatedPetName}</p>}
              </div>
            </div>
          ...
          {/* Removed specific onClick handlers as the whole card is now clickable */}
          <div className="flex gap-2 pt-4">
            <Button
              ...
              // onClick={() => console.log(`Settings for ${name}`)} // Removed onClick
            >
              Configurar
            </Button>
            <Button
              ...
              // onClick={() => console.log(`View data for ${name}`)} // Removed onClick
            >
              Ver Datos
            </Button>
          </div>
